...
if (btc_db_load && {profileNamespace getVariable [format ["btc_hm_%1_db", "PrivateBala"], false]}) then {
    if ((profileNamespace getVariable [format ["btc_hm_%1_version", "PrivateBala"], 1.13]) in [btc_version select 1, 21.1]) then {
